(async () => {
    ...
    function logInfo(message) {
        console.log(`%Magic Mind%c${TEST_ID}-${VARIANT_ID}`, "color:white;background:rgb(0,0,57);font-weight:700;padding:2px 4px;", "margin-left:8px;color:white;background:rgb(0,57,57);font-weight:700;padding:2px 4px;", message);
    }
    ...
    const ANNOUNCE_BAR_SELECTOR = ".shopify-section--announce-bar";

    function getAnnouncementItemMarkup(item) {
        return `
      <div class="${PAGE_INITIALS}__item">
    ...
    function init() {
        const announceBar = q(ANNOUNCE_BAR_SELECTOR);
    ...
    function checkForItems() {
        return !!q(`body:not(.${PAGE_INITIALS})`) && !!q(ANNOUNCE_BAR_SELECTOR);
    }
