...
.PLP-Display_Secondary_Image-swiper .swiper-wrapper {
  height:100%;
...
(() => {
    ...
    const loadSwiper = () => {
        ...
        if (!document.getElementById("swiper-css")) {
            document.head.insertAdjacentHTML(
                "beforeend",
                `<link id="swiper-css" rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@12/swiper-bundle.min.css">`
            );
        }
    ...
    const preloadImages = (images, count = 2) => {
        images.slice(0, count).forEach((src) => { const img = new Image(); img.src = src; });
    };

    const setLoading = (card, on) => { card.style.opacity = on ? "0.6" : ""; };
    ...
    const revealSwiperOnLoad = (swiperEl, existingImg) => {
        ...
        if (firstImg) {
            if (firstImg.complete) {
                ...
                firstImg.onload = () => { existingImg && (existingImg.style.visibility = "hidden"); };
            }
        }
    };
