(() => {
  ...
  function tagProducts() {
    document.querySelectorAll(".byob .byob-product-card").forEach((card) => {
      ...
      card.classList.add("ab-byob-card");
    });
  }
  ...
  function applyFilter() {
    ...
    document.querySelectorAll(".byob .ab-byob-card").forEach((card) => {
      const visible = !items || items.includes(card.dataset.abHandle);
      card.classList.toggle("ab-byob-card--hidden", !visible);
    });

    document.querySelectorAll(".byob .byob-products__collection").forEach((collection) => {
      const hasVisible = collection.querySelector(".ab-byob-card:not(.ab-byob-card--hidden)");
      collection.classList.toggle("ab-byob-collection--hidden", !hasVisible);
    });
  }
  ...
  function selectChip(useCase) {
    ...
    document.querySelectorAll(".ab-usecase-chip").forEach((chip) => {
  ...
  function buildChips() {
    if (document.querySelector(".ab-usecase-filters")) return;

    const chips = Object.entries(useCaseObj)
      .map(
        ([useCase, { icon }], index) => `
        <button type="button" class="ab-usecase-chip" role="radio" aria-checked="false" tabindex="${index === 0 ? 0 : -1}" data-ab-usecase="${useCase}">
          <img class="ab-usecase-chip__icon" src="${icon}" alt="" width="24" height="24" loading="lazy" />
          <span>${useCase}</span>
        </button>`
      )
      .join("");
    ...
    wrapper.className = "ab-usecase-filters";
    wrapper.setAttribute("role", "radiogroup");
    wrapper.setAttribute("aria-label", "Filter by Use case");
    wrapper.innerHTML = chips;

    const section = document.createElement("div");
    section.className = "ab-usecase-section";
    section.appendChild(wrapper);

    document.querySelector(".byob .byob-products").insertAdjacentElement("beforebegin", section);

    wrapper.addEventListener("click", (e) => {
      const chip = e.target.closest(".ab-usecase-chip");
      if (chip) selectChip(chip.dataset.abUsecase);
    });
  }

  /* BYOB re-renders its cards when the discount slider changes, which drops our classes */
