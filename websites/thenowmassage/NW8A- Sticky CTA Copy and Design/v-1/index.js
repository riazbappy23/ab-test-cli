...
`;
    ...
    function injectStyles(cssString) {
        if (document.getElementById("NW8A-styles")) return;
        const style = document.createElement("style");
        style.id = "NW8A-styles";
    ...
    function init() {
        if (!document.body.classList.contains(page_initials)) {
            ...
            anchor.insertAdjacentHTML("afterbegin", `<span class="nw8a-cta__label">${label}</span>${ARROW_SVG}`);
    ...
    function destroyTest() {
        ...
        document.querySelector(".nw8a-cta__label")?.remove();
        document.querySelector(".nw8a-cta__arrow")?.remove();
