(async () => {
  ...
  const ICONS = Object.freeze({
    clock: `<svg class="nw7-svg" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`,
  ...
  const css = `
    ...
    .nw7-row-hours, .nw7-row-address,  .nw7-row-phone {
    ...
    .nw7-hours-btn[aria-expanded="true"] .nw7-caret-svg {
    ...
    .nw7-hours-item.nw7-today {
      color: #fff;
      font-weight: 700;
    }

    .nw7-hours-item .nw7-day-label {
      min-width: 95px;
    }
  ...
  .AB-NW7 .location-hero__cta a{
    ...
    .AB-NW7--v1 .location-info-map__primary-info,
    .AB-NW7--v2 .location-info-map__primary-info {
  ...
  function getStructuredData() {
    ...
    console.warn("No JSON-LD script found");
    return null;
  }
  ...
  function getTodaySpec(specs) {
    return specs.find((s) => {
      const days = Array.isArray(s.dayOfWeek) ? s.dayOfWeek : [s.dayOfWeek];
      return days.some((d) => d.toLowerCase() === TODAY_NAME.toLowerCase());
    });
  }
  ...
  function buildHoursItemsHTML(specs) {
    const seen = new Set();
    return specs
      .flatMap((spec) => {
        ...
        return days
          ...
          .map((day) => {
            const timeStr = `${formatTime(spec.opens)} – ${formatTime(spec.closes)}`;
            return `<div class="nw7-hours-item">
            <span class="nw7-day-label">${day}</span>
            <span>${timeStr}</span>
          </div>`;
          });
      })
  ...
  function getHeroData(data) {
    ...
    const todaySpec = getTodaySpec(specs);
    const openNow = isOpenNow(todaySpec);
    ...
    const mapsUrl = data.hasMap || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(fullAddr)}`;
    ...
    let statusLabel = "";
    if (todaySpec) {
      statusLabel = openNow ? `Open Now · Closes at ${formatTime(todaySpec.closes)}` : `Closed Now - Opens at ${formatTime(todaySpec.opens)}`;
    }
  ...
  function createHeroCard(data) {
    const { specs, line1, line2, mapsUrl, fmtPhone, telHref, statusLabel } = data;
    ...
    card.innerHTML = `
      <div class="nw7-hero-card">
        <div class="nw7-row nw7-row-hours">
          ${ICONS.clock}
          <div class="nw7-hours-row-wrap">
            <button class="nw7-hours-btn" aria-expanded="false" aria-controls="nw7-hours-dd">
              ${statusLabel}
              ${ICONS.caret}
            </button>
            <div class="nw7-hours-dropdown" id="nw7-hours-dd" aria-hidden="true">
              ${buildHoursItemsHTML(specs)}
            </div>
          </div>
        </div>
  ...
  function attachHeroEvents(card) {
    const hoursBtn = card.querySelector(".nw7-hours-btn");
    const dropdown = card.querySelector(".nw7-hours-dropdown");

    hoursBtn.addEventListener("click", (e) => {
      e.stopPropagation();

      const isOpen = hoursBtn.getAttribute("aria-expanded") === "true";

      hoursBtn.setAttribute("aria-expanded", String(!isOpen));
      dropdown.classList.toggle("nw7-open", !isOpen);
      dropdown.setAttribute("aria-hidden", String(isOpen));

      fireGA4Event(`${page_initials}_hours_toggle`, isOpen ? "close" : "open");
    });

    document.addEventListener("click", (e) => {
      if (!card.contains(e.target)) {
        hoursBtn.setAttribute("aria-expanded", "false");
        dropdown.classList.remove("nw7-open");
        dropdown.setAttribute("aria-hidden", "true");
      }
    });

    card.querySelector(".nw7-address-link").addEventListener("click", () => {
      fireGA4Event("NW7_keylocationclicks", "Address");
    });

    card.querySelector(".nw7-phone-link").addEventListener("click", () => {
  ...
  function setupScrollEvent() {
    const heroSection = q(".location-hero__main-container") || q(".location-hero") || q(".location-hero__cta")?.closest("section");
    if (!heroSection) {
  ...
  function setupBTFClickEvents() {
    ...
    if (appointmentContainer) {
      const bookingBtn = appointmentContainer.querySelector("a:first-child");
      if (bookingBtn) {
