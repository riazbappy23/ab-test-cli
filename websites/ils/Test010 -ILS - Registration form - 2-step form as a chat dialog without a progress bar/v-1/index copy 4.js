(function () {
    ...
    const init = () => {
        ...
        if (container) {
            ...
            const street = container.querySelector(".field-street");
            const streetNumber = container.querySelector(".field-streetNumber");
            const zip = container.querySelector(".field-zip");
            const city = container.querySelector(".field-city");
            ...
            const checkStep2 = () => {
                ...
                if (s && sn && z && c) {
                    ...
                    formRoot?.classList.remove("test010-step-3");
                }
            };

            [street, streetNumber, zip, city].forEach((field) => {
                if (field) {
                    const input = field.querySelector("input");
                    input?.addEventListener("input", checkStep2);
                    input?.addEventListener("blur", checkStep2);
                    input?.addEventListener("change", checkStep2);
                }
            });

            const step2Observer = new MutationObserver(checkStep2);
            [street, streetNumber, zip, city].forEach((field) => {
                if (!field) return;
                step2Observer.observe(field, {attributes: true, subtree: true, attributeFilter: ["data-complete", "class"]});
            });
            ...
            if (form) {
                ...
                if (originalButton && actionsWrapper) {
                    ...
                    // Remove custom error message on change for radio buttons
                    const salutationField = form.querySelector(".field-salutation");

                    if (salutationField) {
                        const radios = salutationField.querySelectorAll("input[type='radio']");
                        ...
                        radios.forEach((radio) => {
                            radio.addEventListener("change", () => {
                                ...
                                if (isChecked) {
                                    salutationField.removeAttribute("data-invalid");
                                    salutationField.removeAttribute("data-touched");

                                    const customMsgs = salutationField.querySelectorAll(".test010-custom-msg");
                    ...
                    // Remove custom error messages on submit
